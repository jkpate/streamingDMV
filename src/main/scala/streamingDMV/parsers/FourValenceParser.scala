...
import streamingDMV.parameters.FourValenceParameters

import scala.collection.mutable.{Map=>MMap}
import scala.math.log

class FourValenceParser(
    // maxLength:Int,
    // rootAlpha:Double = 1D,
    // stopAlpha:Double = 1D,
...
) extends FirstOrderFoldUnfoldNOPOSParser[FourValenceParameters](
  ...
  def rightwardSplitSpecs( i:Int, j:Int ) = {
    ...
    if( j == intString.length ) {
      ...
      Seq(
        ...
        // can have zero, one, two, three, or more dependents (top-down) to the right
      Seq(
        (
          NoDependentValence,
          ks.map{ k =>
