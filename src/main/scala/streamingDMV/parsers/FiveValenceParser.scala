...
import streamingDMV.parameters.FiveValenceParameters

import scala.collection.mutable.{Map=>MMap}
import scala.math.log

class FiveValenceParser(
    // maxLength:Int,
    // rootAlpha:Double = 1D,
    // stopAlpha:Double = 1D,
...
) extends FirstOrderFoldUnfoldNOPOSParser[FiveValenceParameters](
  ...
  def cellMap( i:Int, j:Int ) = {
    if( ( i%2 == 1 || j%2 == 1 ) && i%2 != j%2 )
      MMap(
        ...
        ThreeDependentValence -> myZero,
        FourDependentValence -> myZero
      )
    else if( i%2 == 1 && j%2 == 1 )
      MMap(
        ...
        DecorationPair(NoDependentValence,ThreeDependentValence) -> myZero,
        DecorationPair(NoDependentValence,FourDependentValence) -> myZero,
        DecorationPair(OneDependentValence,NoDependentValence) -> myZero,
        DecorationPair(TwoDependentValence,NoDependentValence) -> myZero,
        DecorationPair(ThreeDependentValence,NoDependentValence) -> myZero,
        DecorationPair(FourDependentValence,NoDependentValence) -> myZero
      )
    else
      MMap()
  }
  ...
  def lexCellFactor( index:Int, pDec:Decoration ) = {
    ...
    val score = 
      if( index%2 == 0 ) {
        ...
        assert( pDec != ThreeDependentValence || index >= 6 )
        assert( pDec != FourDependentValence || index >= 8 )
        ...
        assert( pDec != ThreeDependentValence || index <= intString.length-7 )
  ...
  def lexSpecs( index:Int ) = {
    if( index%2 == 0 ) {
      ...
      if( index > 6 )
        Seq( NoDependentValence, OneDependentValence, TwoDependentValence, ThreeDependentValence, FourDependentValence )
      else if( index > 4 )
        Seq( NoDependentValence, OneDependentValence, TwoDependentValence, ThreeDependentValence )
      else if( index > 2 )
      ...
      if( index <= intString.length-9 )
        Seq( NoDependentValence, OneDependentValence, TwoDependentValence, ThreeDependentValence, FourDependentValence )
      else if( index <= intString.length-7 )
        Seq( NoDependentValence, OneDependentValence, TwoDependentValence, ThreeDependentValence )
      else if( index <= intString.length-5 )
        Seq( NoDependentValence, OneDependentValence, TwoDependentValence )
  ...
  def rightwardSplitSpecs( i:Int, j:Int ) = {
    ...
    } else if( j == intString.length-4 ) {
        // can have zero, one, or two dependents (top-down) to the right
      Seq(
        (
          NoDependentValence,
          ks.map{ k =>
        ...
        (
          TwoDependentValence,
          ks.map{ k =>
            ( k, DecorationPair( ThreeDependentValence, NoDependentValence ), NoDependentValence )
          }
        )
      )
    } else if( j == intString.length-6 ) {
        // can have zero, one, two, or three more dependents (top-down) to the right
      Seq(
        (
          NoDependentValence,
          ks.map{ k =>
        ...
        (
          ThreeDependentValence,
          ks.map{ k =>
            ( k, DecorationPair( FourDependentValence, NoDependentValence ), NoDependentValence )
          }
        )
      )
    } else {
        // can have zero, one, two, three, four, or more dependents (top-down) to the right
      Seq(
        (
          NoDependentValence,
          ks.map{ k =>
    ...
    if( i == 0 ) {
      ...
      Seq(
        ...
        (
          TwoDependentValence,
          ks.map{ k =>
            ( k, DecorationPair( NoDependentValence, ThreeDependentValence ), NoDependentValence )
          }
        )
      )
    } else if( i == 6 ) {
      ...
      Seq(
        ...
        (
          ThreeDependentValence,
          ks.map{ k =>
            ( k, DecorationPair( NoDependentValence, FourDependentValence ), NoDependentValence )
          }
        )
      )
    } else {
  ...
  def mSplitSpecs( i:Int, j:Int ) = {
    ...
    val leftMParents =
      if( i >= 7 )
        Seq(
          DecorationPair( NoDependentValence, OneDependentValence ),
          DecorationPair( NoDependentValence, TwoDependentValence ),
          DecorationPair( NoDependentValence, ThreeDependentValence ),
          DecorationPair( NoDependentValence, FourDependentValence )
        )
      else if( i == 5 )
        Seq(
          DecorationPair( NoDependentValence, OneDependentValence ),
    ...
    val rightMParents =
      ...
      //   )
      // else
      if( j <= intString.length-7 )
        Seq(
          DecorationPair( OneDependentValence, NoDependentValence ),
          DecorationPair( TwoDependentValence, NoDependentValence ),
          DecorationPair( ThreeDependentValence, NoDependentValence ),
          DecorationPair( FourDependentValence, NoDependentValence )
        )
      else if( j == intString.length-5 )
        Seq(
          DecorationPair( OneDependentValence, NoDependentValence ),
          DecorationPair( TwoDependentValence, NoDependentValence ),
          DecorationPair( ThreeDependentValence, NoDependentValence )
        )
      else if( j == intString.length-3 )
        Seq(
          DecorationPair( OneDependentValence, NoDependentValence ),
          DecorationPair( TwoDependentValence, NoDependentValence )
        )
